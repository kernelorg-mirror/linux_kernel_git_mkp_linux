...
static int sd_done(struct scsi_cmnd *SCpnt)
{
	...
	if (SCpnt->request->cmd_flags & REQ_WRITE_SAME) {
		/*
		 * Complete entire block range, not just the single
		 * block that was actually transferred.
		 */
		req->__data_len = req->bio->bi_size;

		if (!result) {
			good_bytes = req->__data_len;
			scsi_set_resid(SCpnt, 0);
		} else {
			good_bytes = 0;
			scsi_set_resid(SCpnt, req->__data_len);
		}
	}
